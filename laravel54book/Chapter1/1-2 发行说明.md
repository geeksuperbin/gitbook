...
* [在邮件和通知中支持 Markdown](/docs/laravel/5.4/mail#markdown-mailables)；
* [Laravel Dusk 浏览器自动测试框架](/docs/laravel/5.4/dusk)；
* Laravel Mix；
...
要想了解更多细节，查看 [mail](/docs/laravel/5.4/mail) 和 [notification](/docs/laravel/5.4/notifications) 文档。
...
需要了解更多关于 Dusk 的细节，查看 [Dusk 文档](/docs/laravel/5.4/dusk)
...
需要了解更多关于 components 和 slots 的细节，查看 [Blade documentation](/docs/laravel/5.4/blade)。

### 广播模型绑定

和 HTTP 路由一样，频道路由也使用隐式和显式 [路由模型绑定](/docs/laravel/5.4/routing#route-model-binding)。例如，可以通过请求一个实际的 `Order` 模型实例来取代之前获取字符串或数字订单ID ID：
...
需要了解更多关于广播模型绑定，查看 [事件广播](/docs/laravel/5.4/broadcasting)。
...
Eloquent 事件处理器现在可以被映射到事件对象上。这提供了一种直观处理 Eloquent 事件并易于测试的方式。在开始使用之前，先在你的 Eloquent 模型中定义一个 `$events` 属性数组，在这个数组中可以定义 Eloquent 模型的生命周期中属于你的 [事件 classes](/docs/laravel/5.4/events):
...
需要了更多，查看 [队列](/docs/laravel/5.4/queues).
...
5.4版本以前，只有 Laravel 内置的服务才可以提供 [Facades](/docs/laravel/5.4/facades)，提供快速的，简短的方式访问这些服务容器的方法。在 Laravel 5.4 中，你可以轻松的将你的任意类库实时的转换成Facades，只需要将类名导入到 `Facades` 中。例如，假设你的应用中有这样一个类：
...
当然，如果你以这种方式实现实时 Facades ，就可以使用 Laravel 的 [Facades 模拟功能](/docs/laravel/5.4/mocking) 编写测试用例:
...
更多关于 Laravel 中 Redis 的信息，查看 [Redis 文档](/docs/laravel/5.4/redis)。
...
* [消息通知 Laravel Notifications](/docs/laravel/5.3/notifications)；
* [事件广播 Laravel Echo](/docs/laravel/5.3/broadcasting)；
* [OAuth2 授权认证 Laravel Passport](/docs/laravel/5.3/passport)；
* [全文搜索引擎 Laravel Scout](/docs/laravel/5.3/scout)；
...
[Laravel 社区](http://laravel-notification-channels.com) 已经为通知系统编写了各式的驱动，甚至包括对 iOS 和 Android 通知的支持，更多关于通知系统的信息，请查看 [完整的文档](/docs/laravel/5.3/notifications)。
...
了解更多关于 Echo 和事件广播的信息，请查阅 [完整文档](/docs/laravel/5.3/broadcasting).
...
更多关于 Passport 信息，请查看 [完整文档](/docs/laravel/5.3/passport)。

### 搜索系统 Laravel Scout

Laravel Scout 提供了一个简单的、基于驱动的、针对 [Eloquent](/docs/laravel/5.3/eloquent) 模型的全文搜索解决方案。
...
更多 Scout 功能，请查阅 [Scout 的完整文档](/docs/laravel/5.3/scout)。
...
更多关于 Mailable 的信息，请查看 [完整文档](/docs/laravel/5.3/mail)。
...
完整文档请见 [Laravel Elixir](/docs/laravel/5.3/elixir) 。
...
更多信息，请查看对应文档 [前端文档](/docs/laravel/5.3/frontend)。
...
更多信息请参见 [Artisan 文档](/docs/laravel/5.3/artisan#closure-commands)。
...
更多信息请查看 [Blade 文档](/docs/laravel/5.3/blade#the-loop-variable).
